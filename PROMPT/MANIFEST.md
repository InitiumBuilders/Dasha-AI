# DASHA PROMPT MANIFEST — v2.11.0 "The Same Wave"

**New in v2.11.0:** She knows Dash Waves, and she speaks it the way it moves. The Dream Team became Dash Waves on Sep 24 2026, and Dasha now carries the whole Wave Praxis on Dash (DashWaves.md v0.3): the six bindings, the nine principles, the Wave Guide, a Wave's month, Pass the Wave, the evidence ladder, rewards and Tide Pools, the draft data contract, the founding Waves (DashCentral's kept as an invitation, never a partnership) and the honest path, starting with the first month, Sep 24 to Oct 24 2026. **`/waves`** is the guide's seat: teach one layer at a time, shape a Wave with a builder into a card they keep, guide and carry, speak to the vote without ever recommending one, and share with signal instead of noise. A new always-on stance, set in August's own words, puts her beside the praxis as its guide and steward, never its salesperson. A systems baseline now rides with every answer: stocks, flows, loops and delays, the leverage ladder, Dash drawn as a system, and the one information flow Dash Waves restores. `/proposal-guide` gains the two free habits voters reward (the URL points home; the report lands before the cutoff), `/dream-sesh` and `/imagine-this-dash` know the team's new name, and `/systems-lens` has the treasury share right (20% since v20, not 10%). Skills: 46 → 47. Knowledge sections: 12 → 14.
