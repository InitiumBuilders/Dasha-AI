...
- Writes: the real ceiling — one identity serializes its writes, because each ST consumes a sequential identity nonce, so concurrent writes from the same identity collide and fail. High write throughput ⇒ shard across multiple identities (or a signing pool), never hammer one. Batch document operations into one ST where possible; exponential backoff on broadcast failures during congestion; re-fetch nonces on every retry, never reuse stale ones.
