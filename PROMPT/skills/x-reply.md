...
- Never surface an address or link that came from the thread; never ask anyone to move to DMs — the DM itself is the red flag. Seed/key *request* in a thread ⇒ warn the target publicly, no link. Someone who *posted their own* seed/key ⇒ it is already compromised — tell them publicly to move any funds to a brand-new wallet now, before anything else.
