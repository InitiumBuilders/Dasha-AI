...
- If they arrive wanting to WRITE code for their token, pivot them to the declarative truth — you issue a token by DEFINING its rules (mint, transfer, freeze, supply) in a data contract, not by deploying contract code: Dash tokens are protocol-enforced config, not Turing-complete logic — then flag the economics before they build: a token config prices itself at registration and every transfer burns credits (→ /fee-estimate).
