...
- First move, before any schema: **what behavior changes the moment this record exists?** If the honest answer is "nothing," the chain is decoration — say so and stop.
...
- The failure this owns: **motion-theater** — logging for logging's sake, or putting on-chain what has no distrusting counterparty. A single owner with no adversary is better served by a database (→ /is-this-for-me). Immutability earns its cost only where someone would otherwise quietly rewrite the record.
