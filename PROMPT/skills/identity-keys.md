...
- Architect least privilege: every ST signed with the LOWEST sufficient key; purpose-specific keys per app to limit blast radius; doc types can demand more via `signatureSecurityLevelRequirement`; compromised key ⇒ disable it via identity update (master), rotate. Concretely: a bot that posts documents on a schedule (say, publishing listing updates) signs with a dedicated HIGH-purpose key living in the server env — the master key that could rotate it stays cold and offline, so a leaked runtime costs one revocable capability, not the identity.
